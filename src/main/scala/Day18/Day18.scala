...
object Day18 {
    ...
    def main(args: Array[String]): Unit = {
        val data = io.Source.fromResource("Day18/input").getLines().toSeq
        println(part1(data) + " (correct: 3051)")
        println(part2(data) + " (correct: 4812)")
    }

    def part1(input: Seq[String]): Int = {
        magnitude(input.map(read).reduce(add))
    }

    def part2(input: Seq[String]): Int = {
        input.map(read).toSet.subsets(2)
            .flatMap { set => Seq((set.head, set.last), (set.last, set.head)) }
            .map(add).map(magnitude).max
    }

    def read(input: String): Node = {
        def parseNode(string: String): (Node, String) = {
            string.head match
    ...
    def split(root: Node): Option[Node] = {
        def helper(node: Node): Option[Node] = {
            node match
    ...
    def explode(root: Node): Option[Node] = {
        def helper(node: Node, depth: Int, path: List[Node]): Option[List[Node]] = {
            node match
                case Leaf(_) => None
    ...
    def doExplode(root: Node)(exploder: List[Node]): Node = {
        ...
        @tailrec
        def upLeft(path: List[Node]): Option[Node] = {
            path match
        ...
        @tailrec
        def upRight(path: List[Node]): Option[Node] = {
            path match
